...
class ConditionalRandomField:
    ...
    class Chain:
        ...
        def calculateprobabilityofstates(self,weights):
            num = np.zeros(len(entities.keys())) 
            for t in range(0, self.T):
                num = num + np.multiply(weights, self.featureMap(t))
    ...
    def train(self, alpha=0.1):
        ...
        for its in range(0, 1000):
            ...
            chainProb = 0
            for chain in self.chains:
                for weight in combinations(self.weights,9):
                    p = chain.calculateprobabilityofstates(weight)
                    #print(p)
                    features = np.zeros(self.featureSize)
                    for t in range(0, chain.T):
                        features = features + chain.featureMap(t)
            ...
            self.weights = self.weights + (alpha * (empirical - chainProb)) - self.regularize(self.weights)
